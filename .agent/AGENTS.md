...
## 9. AIとエンジニアの共同開発ルール（開発ガイドライン）

AIエージェントと人間が円滑かつ安全にペアプログラミング・共同開発を進めるための基本ルールです。

### 9.1 基本方針・コミュニケーション
1. **独断での仕様変更・破壊的変更の禁止**:
   - DBスキーマの破壊的マイグレーションや、主要画面の仕様変更、依存パッケージの大規模アップデートを行う際は、必ず事前に方針を提案し、合意を得てから着手する。
2. **小さなステップでの実装と頻繁な検証**:
   - 一度に巨大な変更を行わず、機能単位・モジュール単位で小さく実装し、各ステップごとに動作確認を行う。
3. **説明責任と透明性**:
   - 「なぜそのライブラリ・実装方針を選択したのか」「メリットとトレードオフ」を簡潔かつ明確に伝える。

### 9.2 コード品質・Django設計規約
1. **Djangoのベストプラクティス遵守**:
   - **Fat Model / Thin View**: ビジネスロジックや集計処理はできるだけModelメソッドやManager/QuerySet、サービス層に寄せる。
   - **N+1問題の防止**: リレーション取得時は `select_related` / `prefetch_related` を適切に使用する。
2. **可読性・保守性の確保**:
   - Pythonの型ヒント（Type Hint）を適宜付与し、明瞭で意図が伝わる変数名・関数名を採用する。
   - 不要なコード、コメントアウトしたデバッグコード、不要な `print()` は残さない。
3. **セキュリティ & 機密情報保護**:
   - `SECRET_KEY`、DBパスワード、AWSクレデンシャル等の機密情報は絶対にコードにハードコードしない。必ず `.env` などの環境変数で管理する。
   - SQLインジェクション、XSS、CSRFへの対策を徹底する（Django標準のテンプレートエンジンやORM機能を適切に使う）。

### 9.3 テスト & 検証ルール
1. **変更後のテスト実行**:
   - コードを変更・リファクタリングした後は、関連テスト (`python manage.py test`) を実行し、既存機能が回帰していないことを確認する。
2. **新規機能の実装とテストのセット化**:
   - 新しいビュー、モデル、API、ビジネスロジックを追加した際は、対応するテストケースを合わせて作成する。

### 9.4 ドキュメント同期ルール
1. **仕様変更時の即時反映**:
   - データモデルの変更、画面の追加、インフラ構成の変更を行った際は、本ファイル (`.agent/AGENTS.md`) および `README.md` を最新状態に同期する。
